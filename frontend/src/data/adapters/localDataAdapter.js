...
export function createLocalDataAdapter(store) {
	...
	function linkSearch(doctype, options = {}) {
		...
		const searchFields = Array.from(
			new Set(options.searchFields || [labelField, valueField, "name"])
		);
	...
	async function update(doctype, name, values = {}) {
		if (doctype === "Project") {
			...
			if (values.expected_start_date !== undefined)
		...
		if (doctype === "Task Progress Entry") {
			...
			if (values.unskilled_labour !== undefined)
				patch.unskilledLabour = values.unskilled_labour;
