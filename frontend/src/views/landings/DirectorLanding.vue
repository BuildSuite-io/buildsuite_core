  ...
  <LandingShell>
    <div class="max-w-7xl mx-auto px-6 py-10">
      ...
      <div class="flex items-center gap-3 mb-2">
        ...
        <p class="text-xs text-ink-500 uppercase tracking-wider font-medium">{{ role.shortName }} · Portfolio overview</p>
      </div>
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-3 mt-8">
        ...
        <div class="bg-white border border-ink-200 rounded-xl p-4">
          <div class="text-[11px] uppercase tracking-wider text-ink-500 font-medium">Total order book</div>
          <div class="text-2xl font-semibold text-ink-900 mt-1">{{ fmtCompactINR(store.totalOrderBook) }}</div>
        </div>
        <div class="bg-white border border-ink-200 rounded-xl p-4">
          <div class="text-[11px] uppercase tracking-wider text-ink-500 font-medium">Avg variance</div>
          <div class="text-2xl font-semibold mt-1 tabular-nums" :class="variancePill(avgVariancePct)">
            {{ avgVariancePct > 0 ? '+' : '' }}{{ avgVariancePct.toFixed(1) }}%
          </div>
          <div class="text-[11px] text-ink-400 mt-0.5">{{ avgVariancePct > 0 ? 'behind' : 'ahead of' }} plan, schedule basis</div>
        </div>
        <div class="bg-white border border-ink-200 rounded-xl p-4">
            ...
            >
              <div class="flex items-center gap-3">
                <div class="flex-1 min-w-0">
                  ...
                  <div class="text-xs text-ink-500 mt-0.5">{{ p.client }} · {{ fmtCompactINR(p.budget) }} · {{ p.days > 0 ? `${p.days}d to deadline` : `${-p.days}d overdue` }}</div>
                </div>
