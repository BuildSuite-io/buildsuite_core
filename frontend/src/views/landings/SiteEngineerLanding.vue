...
const inProgressCount = computed(
	() => myTasks.value.filter((t) => t.status === "In Progress").length
);

const pendingScosOnPrimary = computed(() => {
	if (!primaryProject.value) return 0;
	return store
		.scosByProject(primaryProject.value.id)
		.filter((s) => s.status === "Pending Approval").length;
});
...
const quickActions = computed(() => [
	{ icon: "👷", label: "Mark attendance", to: "/workforce", sub: "Open Workforce" },
	{
		...
		to: "/tasks",
		sub: `${inProgressCount.value} task${inProgressCount.value === 1 ? "" : "s"} in progress`,
	},
	{ icon: "🧾", label: "Raise material request", to: "/procurement", sub: "Open Procurement" },
	{
		icon: "⚠️",
		label: "Report issue / SCO",
		to: "/sco",
		sub:
			pendingScosOnPrimary.value === 0
				? "No SCOs pending on this project"
				: `${pendingScosOnPrimary.value} pending on this project`,
	},
]);
...
const alerts = computed(() => {
	...
	wpIds.forEach((wpId) => {
		...
		const elapsed = Math.max(
			0,
			Math.min(total, today.getTime() - new Date(wp.startDate).getTime())
		);
...
<template>
	<LandingShell>
		<div class="max-w-2xl mx-auto px-5 py-8">
			...
			<div
				class="rounded-2xl p-5 text-white shadow-fp-lg"
				style="background-image: linear-gradient(135deg, #16a34a 0%, #15803d 100%)"
			>
				<div
					class="flex items-center gap-2 text-xs opacity-90 uppercase tracking-wider font-medium"
				>
				...
				<div class="divide-y divide-ink-100">
					...
					>
						...
						<div class="flex-1 min-w-0">
							...
							<div v-if="t.wpName" class="text-[11px] text-ink-500 mt-0.5 truncate">
								{{ t.wpName }}
							</div>
						</div>
						<div class="text-right flex-shrink-0">
							<div class="text-xs text-ink-700 tabular-nums font-medium">
								{{ t.progress }}%
							</div>
							<div class="text-[10px] text-ink-500 uppercase tracking-wider">
								{{ t.priority }}
							</div>
						</div>
					</RouterLink>
			...
			<div
				v-else
				class="bg-success-50 border border-success-200 rounded-xl px-4 py-3 text-success-800 text-sm"
			>
				<span class="font-medium">All clear ✓</span>
				<span class="text-success-700 ml-1"
					>· No overdue tasks or behind-schedule work packages.</span
				>
			</div>
		</div>
	</LandingShell>
