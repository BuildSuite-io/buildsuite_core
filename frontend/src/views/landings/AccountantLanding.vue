...
const ILLUSTRATIVE_KPIS = computed(() => [
	...
	{ label: "RA bills to pay", value: "11", sub: "Subcontract" },
	{
		...
		value: orderBookCompact.value + " booked",
		sub: "Order book booked",
	},
...
const pendingPayments = [
	...
	{
		id: "PI-2026-0156",
		vendor: "Stone & Aggregate Suppliers",
		category: "Material",
		amount: 318000,
		dueIn: 11,
	},
		...
		<div class="max-w-5xl mx-auto px-6 py-10">
			...
			<p class="text-sm text-ink-500 mt-1">
				...
				<span class="text-ink-700 font-medium"
					>{{ pendingPayments.length }} vendor payments pending</span
				>
			</p>
				...
				<div class="divide-y divide-ink-100">
					...
					>
						<div
							...
							:class="
								p.category === 'Subcontract'
									? 'bg-info-50 text-info-700'
									: 'bg-warning-50 text-warning-700'
							"
						>
							{{ p.category === "Subcontract" ? "SC" : "PI" }}
						</div>
						...
						<div class="text-right flex-shrink-0">
							<div class="text-sm font-semibold text-ink-900 tabular-nums">
								{{ fmtCompactINR(p.amount) }}
							</div>
							<div
								class="text-[11px]"
								:class="p.dueIn <= 7 ? 'text-warning-700' : 'text-ink-500'"
							>
								due in {{ p.dueIn }}d
							</div>
						</div>
