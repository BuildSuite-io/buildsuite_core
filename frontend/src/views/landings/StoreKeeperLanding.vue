...
const todaysGRNs = [
  { id: 'GRN-2026-0421', supplier: 'UltraTech Cement Ltd.',   item: 'Cement OPC 53',         qty: '200 bags',  expectedAt: '10:30' },
  { id: 'GRN-2026-0422', supplier: 'JSW Steel',                item: 'TMT Fe500 — 12mm',      qty: '4.5 MT',    expectedAt: '12:00' },
  { id: 'GRN-2026-0423', supplier: 'Prism Johnson',            item: 'RMC M30',               qty: '18 m³',     expectedAt: '14:00' },
  { id: 'GRN-2026-0424', supplier: 'Sand Suppliers Co-op',     item: 'River sand',            qty: '24 m³',     expectedAt: '15:30' },
  { id: 'GRN-2026-0425', supplier: 'Local Brick Kiln Network', item: 'Common burnt bricks',   qty: '8,000 nos', expectedAt: '16:00' },
]
  ...
  <LandingShell>
    <div class="max-w-5xl mx-auto px-6 py-10">
      ...
      <div class="flex items-center gap-3 mb-2">
        ...
        <p class="text-xs text-ink-500 uppercase tracking-wider font-medium">{{ role.shortName }} · Site stores</p>
      </div>
      <h1 class="text-2xl font-semibold text-ink-900 tracking-tight">Stock status today</h1>
      <p class="text-sm text-ink-500 mt-1">
        {{ todayLabel }} ·
        <span class="text-ink-700 font-medium">{{ todaysGRNs.length }} GRN{{ todaysGRNs.length === 1 ? '' : 's' }} expected today</span>
      </p>
        ...
        <div class="divide-y divide-ink-100">
          ...
          >
            ...
            <div class="flex-1 min-w-0">
              <div class="text-sm font-medium text-ink-900 truncate">{{ g.item }} · {{ g.qty }}</div>
              <div class="text-xs text-ink-500 mt-0.5 truncate">{{ g.supplier }} · expected {{ g.expectedAt }}</div>
            </div>
            <span class="text-[10px] text-ink-500 font-mono flex-shrink-0">{{ g.id }}</span>
          </RouterLink>
        </div>
      </div>
