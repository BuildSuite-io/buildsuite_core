...
const myPendingScos = computed(() =>
  store.scos.filter(s => s.status === 'Pending Approval' && myProjectIds.value.has(s.projectId))
)
  ...
  <LandingShell>
    <div class="max-w-5xl mx-auto px-6 py-10">
      ...
      <div class="flex items-center gap-3 mb-2">
        ...
        <p class="text-xs text-ink-500 uppercase tracking-wider font-medium">{{ role.shortName }} · My projects</p>
      </div>
      <h1 class="text-2xl font-semibold text-ink-900 tracking-tight">Good morning</h1>
      <p class="text-sm text-ink-500 mt-1">
        {{ todayLabel }} ·
        <span class="text-ink-700 font-medium">{{ myActiveCount }} active project{{ myActiveCount === 1 ? '' : 's' }}</span> ·
        <span class="text-ink-700 font-medium">{{ pendingApprovalsCount }} pending approval{{ pendingApprovalsCount === 1 ? '' : 's' }}</span>
      </p>
        ...
        >
          <div class="flex items-start justify-between gap-3 mb-3">
            ...
            <div class="text-right flex-shrink-0">
              <div class="text-[11px] text-ink-500">{{ p.days > 0 ? `${p.days}d to deadline` : `${-p.days}d overdue` }}</div>
              <div class="text-[11px] tabular-nums mt-0.5" :class="variancePill(p.variancePct)">
          ...
          <div class="flex items-center gap-4 text-xs text-ink-500">
            <span>{{ p.openTasksCount }} open task{{ p.openTasksCount === 1 ? '' : 's' }}</span>
            <span>·</span>
            <span :class="p.pendingScosCount > 0 ? 'text-warning-700 font-medium' : ''">
              {{ p.pendingScosCount }} pending SCO{{ p.pendingScosCount === 1 ? '' : 's' }}
            </span>
        ...
        <div class="divide-y divide-ink-100">
          ...
          >
            ...
            <div class="flex-1 min-w-0">
              <div class="text-sm font-medium text-ink-900 truncate">{{ a.title }}</div>
              <div class="text-xs text-ink-500 mt-0.5 truncate">{{ a.project }} · raised {{ a.daysAgo }}d ago</div>
            </div>
            <div class="text-right flex-shrink-0">
              <div class="text-sm font-semibold text-ink-900 tabular-nums">{{ fmtCompactINR(a.amount) }}</div>
            </div>
            <UserAvatar :user-id="a.raisedBy" size="xs" />
            <span class="text-brand-600 text-sm flex-shrink-0">Review →</span>
          </RouterLink>
          <div v-if="!approvals.length" class="px-4 py-6 text-center text-sm text-ink-400">
          ...
          <div class="flex items-center gap-3 px-4 py-3 bg-ink-50/50">
            <span class="w-8 h-8 rounded-lg bg-ink-100 text-ink-400 flex items-center justify-center text-[10px] font-semibold tracking-wider">MR</span>
            <div class="flex-1 min-w-0">
              <div class="text-sm text-ink-500">Material requests</div>
              <div class="text-xs text-ink-400 mt-0.5">0 pending · Procurement (M4) — coming soon</div>
            </div>
          </div>
          <div class="flex items-center gap-3 px-4 py-3 bg-ink-50/50">
            <span class="w-8 h-8 rounded-lg bg-ink-100 text-ink-400 flex items-center justify-center text-[10px] font-semibold tracking-wider">PC</span>
            <div class="flex-1 min-w-0">
              <div class="text-sm text-ink-500">Petty cash requests</div>
              <div class="text-xs text-ink-400 mt-0.5">0 pending · Project Finance (M8) — coming soon</div>
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="divide-y divide-ink-100">
          ...
          >
            ...
            <div class="flex items-center gap-2 flex-shrink-0">
              ...
              <span class="text-xs text-ink-700 tabular-nums w-8 text-right">{{ t.progress }}%</span>
            </div>
            <UserAvatar :user-id="t.assignee" size="xs" />
          </RouterLink>
          <div v-if="!criticalTasks.length" class="px-4 py-8 text-center text-sm text-ink-400">
