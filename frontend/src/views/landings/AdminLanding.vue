  ...
  <LandingShell>
    <div class="max-w-6xl mx-auto px-6 py-10">
      ...
      <div class="flex items-center gap-3 mb-2">
        ...
        <p class="text-xs text-ink-500 uppercase tracking-wider font-medium">{{ role.shortName }} · System overview</p>
      </div>
      ...
      <section class="mt-10">
        <div class="flex items-baseline justify-between mb-3">
          ...
          <span class="text-[11px] text-ink-500">{{ buildsuiteTiles.length }} workspace{{ buildsuiteTiles.length === 1 ? '' : 's' }}</span>
        </div>
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          ...
          >
            <div class="flex items-start gap-3">
              <div class="w-10 h-10 rounded-lg bg-brand-50 flex items-center justify-center text-xl flex-shrink-0">{{ t.icon }}</div>
              <div class="flex-1 min-w-0">
                <div class="text-sm font-semibold text-ink-900">{{ t.name }}</div>
                <div class="text-[11px] text-ink-500 mt-0.5 leading-snug">{{ t.desc }}</div>
      ...
      <section class="mt-8">
        <div class="flex items-baseline justify-between mb-3">
          ...
          <span class="text-[11px] text-ink-500">{{ erpnextTiles.length }} workspace{{ erpnextTiles.length === 1 ? '' : 's' }}</span>
        </div>
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          <RouterLink
          ...
          >
            <div class="flex items-start gap-3">
              <div class="w-10 h-10 rounded-lg bg-ink-50 flex items-center justify-center text-xl flex-shrink-0 opacity-80">{{ t.icon }}</div>
              <div class="flex-1 min-w-0">
                <div class="text-sm font-semibold text-ink-700">{{ t.name }}</div>
                <div class="text-[11px] text-ink-500 mt-0.5 leading-snug">{{ t.desc }}</div>
                <div v-if="t.metric" class="text-[11px] text-ink-500 font-medium mt-2">{{ t.metric }}</div>
              </div>
            </div>
          </RouterLink>
        </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 mt-10">
          ...
          <div class="divide-y divide-ink-100">
            ...
            >
              ...
              <div class="flex-1 min-w-0">
                ...
                <div v-if="e.sub" class="text-xs text-ink-500 mt-0.5 truncate">{{ e.sub }}</div>
              </div>
              <span class="text-xs text-ink-500 flex-shrink-0">{{ fmtDate(e.date) }}</span>
            </RouterLink>
            <div v-if="!recentActivity.length" class="px-4 py-8 text-center text-sm text-ink-400">
              No activity yet.
            </div>
          </div>
        </div>
        ...
        <div class="bg-white border border-ink-200 rounded-xl overflow-hidden">
          ...
          <div class="divide-y divide-ink-100">
            ...
            >
              <span class="w-2 h-2 rounded-full mt-1.5 flex-shrink-0 bg-success-500"></span>
              <div class="flex-1 min-w-0">
                <div class="text-[11px] uppercase tracking-wider text-ink-500 font-medium">{{ h.label }}</div>
                <div class="text-xs text-ink-700 mt-0.5 break-words">{{ h.value }}</div>
              </div>
            </div>
          </div>
        </div>
