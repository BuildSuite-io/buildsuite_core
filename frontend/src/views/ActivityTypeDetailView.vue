...
function saveEdit() {
  ...
  const cleanChecklist = (form.value.defaultChecklist || []).filter(c => c && c.item && c.item.trim())
    ...
    <DeskForm>
      ...
      <DeskSection title="Basic" v-if="!editing">
        ...
        <DeskField label="Description">
          <div class="text-sm text-ink-700 py-1 whitespace-pre-line">{{ activityType.description || '—' }}</div>
        </DeskField>
      </DeskSection>
      ...
      <DeskSection title="Default labour mix" v-if="!editing" :cols="2">
        <DeskField label="Skilled ratio" hint="Of every man-day, this fraction is skilled labour.">
          <div class="text-sm text-ink-900 py-1 tabular-nums">{{ pct(activityType.defaultSkilledRatio) }}%</div>
        </DeskField>
        <DeskField label="Unskilled ratio" hint="Auto-computed as 100 − skilled.">
          <div class="text-sm text-ink-500 py-1 tabular-nums">{{ pct(activityType.defaultUnskilledRatio) }}%</div>
        </DeskField>
      </DeskSection>
      <DeskSection title="Default labour mix" v-else :cols="2">
        <DeskField label="Skilled ratio (0–1)" required hint="0.3 = 30% skilled. Unskilled auto-fills.">
          <DeskInput
      ...
      <DeskSection title="Default productivity" v-if="!editing" :cols="2">
        <DeskField label="Per man-day">
          <div class="text-sm text-ink-900 py-1 tabular-nums">
            {{ activityType.expectedProductivityPerManDay }} {{ activityType.productivityUnit }}
          </div>
        </DeskField>
        <DeskField label="Unit">
          <div class="text-sm text-ink-700 py-1">{{ activityType.productivityUnit || '—' }}</div>
        </DeskField>
      </DeskSection>
      ...
      <DeskSection title="Default checklist">
        <div class="md:col-span-2">
          <div v-if="!editing">
            <ol v-if="activityType.defaultChecklist && activityType.defaultChecklist.length" class="space-y-1.5">
              <li
                v-for="(c, i) in activityType.defaultChecklist"
                :key="i"
                class="flex items-start gap-2 text-sm text-ink-800"
              >
                <span class="text-ink-400 tabular-nums w-5 text-right">{{ i + 1 }}.</span>
                <span>{{ c.item }}</span>
              </li>
            </ol>
            <div v-else class="text-xs text-ink-400 italic">No checklist items defined.</div>
          </div>
          <div v-else>
        ...
        <div class="md:col-span-2">
          <div v-if="!editing" class="flex flex-wrap gap-1.5">
            <span
              ...
              style="border-radius: 2px;"
            >{{ t }}</span>
            <span v-if="!(activityType.applicableProjectTypes && activityType.applicableProjectTypes.length)" class="text-xs text-ink-400 italic">Universal · applies to all project types</span>
          </div>
          <div v-else>
      ...
      <section class="mt-8 pt-4 border-t border-ink-200">
        <div class="flex items-center gap-6 text-xs text-ink-500 flex-wrap">
          <div class="flex items-center gap-1.5">
            <span>🔗</span>
            <span>Linked tasks — <span class="font-medium text-ink-700">{{ linkedTaskCount }}</span></span>
          </div>
          <div class="flex items-center gap-1.5">
            <span>💬</span>
