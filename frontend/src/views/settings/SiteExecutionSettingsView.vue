...
watch(() => store.siteExecutionSettings, (s) => { if (s) form.value = JSON.parse(JSON.stringify(s)) }, { immediate: true, deep: true })
  ...
  >
    <DeskForm>
      ...
      <DeskSection title="Task defaults">
        <DeskField label="Default task type" hint="Pre-fills task type on new tasks. Choose Activity, Milestone or Inspection.">
          <div v-if="!editing" class="text-sm text-ink-900 py-1">{{ store.siteExecutionSettings.default_task_type }}</div>
      ...
      <DeskSection title="Company propagation">
        ...
        >
          ...
          <label v-else class="flex items-center gap-2 py-1 text-sm cursor-pointer">
            <input type="checkbox" v-model="form.auto_propagate_company" class="accent-brand-600" />
      ...
      <DeskSection title="Task Progress Entry">
        ...
        >
          <div v-if="!editing" class="text-sm text-ink-900 py-1">
            {{ store.siteExecutionSettings.tpe_attachment_required ? 'Required' : 'Optional' }}
          </div>
          <label v-else class="flex items-center gap-2 py-1 text-sm cursor-pointer">
            <input type="checkbox" v-model="form.tpe_attachment_required" class="accent-brand-600" />
            <span>{{ form.tpe_attachment_required ? 'Required' : 'Optional' }}</span>
          </label>
        </DeskField>
      </DeskSection>
      ...
      <DeskSection title="Safety">
        ...
        >
          <div v-if="!editing" class="text-sm text-ink-900 py-1">
            {{ store.siteExecutionSettings.cascade_delete_confirmation ? 'On' : 'Off' }}
          </div>
          <label v-else class="flex items-center gap-2 py-1 text-sm cursor-pointer">
            <input type="checkbox" v-model="form.cascade_delete_confirmation" class="accent-brand-600" />
