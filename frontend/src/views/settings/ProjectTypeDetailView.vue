  ...
  >
    <DeskForm>
        ...
        >
          <template #left>
            <span class="text-[11px] text-ink-500">
              {{ projectsUsingThis.length }} project{{ projectsUsingThis.length === 1 ? '' : 's' }} use this type.
            </span>
          </template>
          <template #menu>
            <button
      ...
      <DeskSection title="Basic">
        ...
        <DeskField label="Sort order" hint="Controls the order this type appears in the new-project type dropdown.">
          <div v-if="!editing" class="text-sm text-ink-900 py-1 tabular-nums">{{ record.sort_order || '—' }}</div>
          <DeskInput v-else v-model="form.sort_order" type="number" />
        </DeskField>
        <DeskField label="Enabled" hint="Disabled types are hidden from the new-project type dropdown. Existing projects keep their type and settings.">
          <div v-if="!editing" class="text-sm text-ink-900 py-1">{{ record.enabled ? 'Enabled' : 'Disabled' }}</div>
      ...
      <DeskSection title="Work Package label">
        ...
        >
          <div v-if="!editing" class="text-sm text-ink-900 py-1">{{ record.workPackageLabel || '— inherits default ("Work Package") —' }}</div>
          <DeskInput v-else v-model="form.workPackageLabel" placeholder="Work Package" />
        </DeskField>
        <DeskField label="Plural" hint="Used in sidebar entries, page titles and list subtitles.">
          <div v-if="!editing" class="text-sm text-ink-900 py-1">{{ record.workPackageLabelPlural || '— inherits default ("Work Packages") —' }}</div>
          <DeskInput v-else v-model="form.workPackageLabelPlural" placeholder="Work Packages" />
        </DeskField>
      </DeskSection>
      ...
      <DeskSection title="Default template">
        ...
        >
          <div v-if="!editing" class="py-1">
            <span v-if="record.defaultTemplate" class="text-sm px-2 py-0.5 bg-ink-100 text-ink-700" style="border-radius: 9999px;">{{ record.defaultTemplate }}</span>
            <span v-else class="text-sm text-ink-400 italic">No template — projects of this type are created empty.</span>
          </div>
        ...
        <DeskField label="Template preview" v-if="templatePreview">
          <div class="px-3 py-2 bg-ink-50 border border-ink-200 text-[11px] text-ink-700 space-y-1" style="border-radius: 6px;">
            <div>
              <span class="font-medium text-ink-900">{{ (templatePreview.defaultStages || []).length }}</span> default stages:
              <span class="text-ink-600">{{ (templatePreview.defaultStages || []).map(s => s.stageName).join(' → ') }}</span>
            </div>
            <div>
              <span class="font-medium text-ink-900">{{ (templatePreview.defaultWorkPackages || []).length }}</span> default work packages:
              <span class="text-ink-600">{{ (templatePreview.defaultWorkPackages || []).map(w => w.name).join(' · ') }}</span>
            </div>
            <div>
              <span class="font-medium text-ink-900">{{ (templatePreview.defaultTasks || []).length }}</span> default tasks across those work packages.
            </div>
          </div>
        </DeskField>
      </DeskSection>
