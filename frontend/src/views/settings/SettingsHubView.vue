    ...
    <div class="flex items-center gap-3 mb-5 p-3 border border-ink-200 bg-ink-50" style="border-radius: 6px;">
      ...
      <div class="flex-1 min-w-0">
        <div class="text-sm text-ink-900">
          Signed in as <span class="font-medium">{{ store.user?.name }}</span>
          · role <span class="font-medium text-ink-700">{{ store.currentRole?.name }}</span>
          <span v-if="store.isMultiCompany && store.currentCompany"> · acting on <span class="font-medium text-ink-700">{{ store.currentCompany.shortName }}</span></span>
        </div>
      </div>
    </div>

    <div v-for="group in visibleGroups" :key="group.title" class="mb-6">
      <h2 class="text-[11px] font-semibold uppercase tracking-wider text-ink-700 mb-2">{{ group.title }}</h2>
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
        <div
          v-for="tile in group.tiles"
          :key="tile.slug"
          class="border border-ink-200 bg-white p-3"
          :class="tile.stub ? 'opacity-90' : 'hover:border-brand-400 hover:bg-brand-50 cursor-pointer'"
        ...
        >
          <div class="flex items-start gap-3">
            ...
            <div class="flex-1 min-w-0">
              ...
              <div class="text-[11px] text-ink-600 mt-0.5 leading-snug">{{ tile.desc }}</div>
            </div>
          </div>
        </div>
      </div>
