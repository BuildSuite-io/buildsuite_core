...
watch(() => store.coreSettings, (s) => { if (s) form.value = JSON.parse(JSON.stringify(s)) }, { immediate: true, deep: true })
  ...
  >
    <DeskForm>
      ...
      <DeskSection title="Multi-company">
        ...
        >
          <div v-if="!editing" class="text-sm text-ink-900 py-1">
            {{ store.coreSettings.enable_company_segregation ? 'Enabled' : 'Disabled' }}
          </div>
          <label v-else class="flex items-center gap-2 py-1 text-sm cursor-pointer">
            <input type="checkbox" v-model="form.enable_company_segregation" class="accent-brand-600" />
            <span>{{ form.enable_company_segregation ? 'Enabled' : 'Disabled' }}</span>
          </label>
        </DeskField>
        <DeskField label="Default company" hint="The company pre-selected on Project create when the user doesn't pick one explicitly. Must exist in the Companies fixture.">
          <div v-if="!editing" class="text-sm text-ink-900 py-1">
            {{ store.companyById(store.coreSettings.default_company)?.name || store.coreSettings.default_company }}
          </div>
      ...
      <DeskSection title="Project defaults">
        <DeskField label="Default project type" hint="Pre-fills the Project type field on new projects.">
          <div v-if="!editing" class="text-sm text-ink-900 py-1">{{ store.coreSettings.default_project_type }}</div>
