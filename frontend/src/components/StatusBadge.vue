...
const classes = computed(() => {
  const map = {
    ...
    // Task status (custom task_status field): Yet To Start / In Progress / In Delay / Completed
  ...
  return `${base} rounded-full font-medium inline-flex items-center ${map[props.status] || 'bg-ink-100 text-ink-600'}`
})
</script>
