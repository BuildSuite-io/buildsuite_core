...
const _COLORS = ['#F57F17', '#6A1B9A', '#0277BD', '#2E7D32', '#C62828', '#00695C', '#4527A0', '#558B2F']
...
async function resolveUser(userId) {
  if (!userId) { userInfo.value = null; return }
  ...
  if (_cache.has(userId)) { userInfo.value = _cache.get(userId); return }
...
<template>
  <div class="inline-flex items-center gap-1.5 min-w-0">
    <!-- Avatar circle -->
    <div
      :class="[sizeClass, 'rounded-full flex-shrink-0 overflow-hidden flex items-center justify-center font-medium']"
      :style="userInfo?.image ? '' : `background-color:${userInfo?.color ?? colorForId(userId)}`"
    >
      <img
        v-if="userInfo?.image"
        :src="userInfo.image"
