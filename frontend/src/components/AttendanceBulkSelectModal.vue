<script setup>
// Bulk Select Employees. Workers already on the sheet show as "On sheet" and are
// not selectable — the modal only adds, so a checkbox you could untick with no
// effect would lie.
...
function addWorkerRows(workers) {
	const known = new Set(workerOptions.value.map((o) => o.value));
	const already = existingSet.value;
	const names = new Map(extraNames.value);
	const s = new Set(checked.value);
	for (const w of workers) {
		if (!w.employee || already.has(w.employee)) continue;
		s.add(w.employee);
...
function selectAll() {
	const s = new Set(checked.value);
	workerOptions.value.forEach((o) => {
		if (!existingSet.value.has(o.value)) s.add(o.value);
	});
	checked.value = s;
}
					...
					>
						...
						>
							<input
								...
								:disabled="existingSet.has(e.value)"
