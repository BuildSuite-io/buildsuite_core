...
watch(() => props.projectId, (newVal) => {
  ...
  tasksResource.value = adapter.list('Task', {
    fields: ['name', 'subject', 'project', 'work_package', 'task_status', 'task_type', 'owner', 'exp_start_date', 'exp_end_date'],
...
watch(() => props.projectId, (newVal) => {
  if (!newVal) { wpsResource.value = null; return }
...
watch(() => props.projectId, (newVal) => {
  if (!newVal) { stagesResource.value = null; return }
...
function suggestByDateOverlap() {
  ...
  if (count === 0) {
    ...
    showToast(`${count} task${count === 1 ? '' : 's'} suggested based on date overlap. Review and adjust.`)
    const expanded = new Set(expandedGroups.value)
    for (const g of wpGroups.value) {
      if (g.tasks.some(t => selectedIds.value.has(t.id))) expanded.add(g.key)
    }
    if (unassignedGroup.value && unassignedGroup.value.tasks.some(t => selectedIds.value.has(t.id))) {
      expanded.add('__unassigned__')
    }
    expandedGroups.value = expanded
  }
...
<template>
      ...
      >
        ...
        >
          <div class="min-w-0 flex-1">
            <h2 class="text-sm font-semibold text-ink-900 dark:text-[#F5F5F5]">Add tasks to stage</h2>
        ...
        <div class="flex-1 overflow-y-auto">
          ...
          <div class="px-5 pt-4 pb-3 border-b border-ink-100 bg-white sticky top-0 z-10 dark:bg-[#242424] dark:border-ink-700">
            <div class="flex items-center justify-between gap-3 flex-wrap">
              <button
                ...
                :title="suggestionDisabled ? 'Set stage dates to use suggestion.' : 'Suggest tasks whose dates overlap the stage window.'"
                @click="suggestByDateOverlap"
              >Suggest tasks by date overlap</button>
          ...
          <div class="px-5 py-4 space-y-3">
            ...
            >
              No tasks created on this project yet. Create tasks in the Tasks tab before adding them to a stage.
            </div>
            ...
            <template v-else>
              ...
              >
                ...
                >
                  <span
                    ...
                    :style="{ transform: expandedGroups.has(group.key) ? 'rotate(90deg)' : 'rotate(0deg)' }"
                  >▶</span>
                  <input
                    type="checkbox"
                  ...
                  />
                  <span class="text-sm font-medium text-ink-900 dark:text-[#F5F5F5]">{{ group.title }}</span>
                  <span class="text-[11px] text-ink-500 dark:text-ink-400">· {{ group.tasks.length }} task{{ group.tasks.length === 1 ? '' : 's' }}</span>
                </div>

                <!-- Task rows -->
                <div v-if="expandedGroups.has(group.key)" class="border-t border-ink-100 dark:border-ink-700">
                  <div
                  ...
                  >
                    ...
                    <div class="min-w-0 flex-1">
                      <div class="flex items-center gap-2 flex-wrap">
                        <span class="text-sm font-medium text-ink-900 dark:text-[#F5F5F5] truncate">{{ task.name }}</span>
                        <StatusBadge v-if="task.status" :status="task.status" size="xs" />
                        <StatusBadge v-if="task.task_type" :status="task.task_type" size="xs" />
                      </div>
                      <div class="text-[11px] text-ink-500 dark:text-ink-400 mt-1">
                        <template v-if="task.startDate || task.endDate">
                          {{ task.startDate ? fmtDate(task.startDate) : '—' }} – {{ task.endDate ? fmtDate(task.endDate) : '—' }}
                        </template>
              ...
              >
                ...
                >
                  <span
                    class="inline-block text-ink-500 text-[10px] transition-transform"
                    :style="{ transform: expandedGroups.has(unassignedGroup.key) ? 'rotate(90deg)' : 'rotate(0deg)' }"
                  >▶</span>
                  <input
                    type="checkbox"
                    class="cursor-pointer accent-brand-600"
                    :checked="groupSelectionState(unassignedGroup) === 'all'"
                    :ref="(el) => setIndeterminate(el, groupSelectionState(unassignedGroup))"
                    @click.stop
                    @change="toggleGroup(unassignedGroup)"
                  />
                  <span class="text-sm font-medium text-ink-900 dark:text-[#F5F5F5]">{{ unassignedGroup.title }}</span>
                  <span class="text-[11px] text-ink-500 dark:text-ink-400">· {{ unassignedGroup.tasks.length }} task{{ unassignedGroup.tasks.length === 1 ? '' : 's' }}</span>
                </div>
                <div v-if="expandedGroups.has(unassignedGroup.key)" class="border-t border-ink-100 dark:border-ink-700">
                  <div
  ...
  <!-- ============= EMBEDDED MODE ========================================== -->
  <div v-else class="bg-white border border-ink-200 dark:bg-[#242424] dark:border-ink-700" style="border-radius: 8px;">
    <!-- Top bar -->
    <div class="px-5 pt-4 pb-3 border-b border-ink-100 dark:border-ink-700">
      <div class="flex items-center justify-between mb-3">
        <div class="min-w-0 flex-1">
          ...
          <p v-if="plannedStart && plannedEnd" class="text-[11px] text-ink-500 mt-0.5 dark:text-ink-400">
            {{ fmtDate(plannedStart) }} – {{ fmtDate(plannedEnd) }}
          </p>
        </div>
        <span class="text-xs text-ink-700 tabular-nums dark:text-ink-400">
