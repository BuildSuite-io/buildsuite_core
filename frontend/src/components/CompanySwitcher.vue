...
<template>
  <div v-if="store.isMultiCompany && currentCompany" class="relative">
    ...
    >
      ...
      <span class="text-xs font-medium text-ink-700 whitespace-nowrap">{{ currentCompany.shortName }}</span>
    ...
    <template v-if="open">
      ...
      >
        ...
        <div class="max-h-[65vh] overflow-y-auto scrollbar-thin">
          ...
          >
            <span :class="c.color" class="w-2.5 h-2.5 rounded-full mt-1.5 flex-shrink-0"></span>
            <div class="flex-1 min-w-0">
              <div class="text-sm font-medium text-ink-900 leading-tight">{{ c.name }}</div>
              <div class="text-[11px] text-ink-500 mt-0.5 leading-snug">{{ c.description }}</div>
            </div>
            <span
