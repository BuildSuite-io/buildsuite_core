...
// A value missing from `options` (filtered out, or still loading) shows the
// stored id, not the placeholder — the placeholder would assert the opposite of
// the truth and hide the clear button.
const hasValue = computed(() => props.modelValue !== "" && props.modelValue != null);
const triggerLabel = computed(() => {
	if (selected.value) return selected.value.label;
	return hasValue.value ? String(props.modelValue) : "";
});
...
function positionPopover() {
	...
	// Viewport-aware placement. Near the bottom of the page the panel flips UPWARD
	// instead of running off-screen; either way its height clamps to the space
	// available so the list scrolls internally.
	const MAX_H = 360;
	...
	if (spaceBelow >= Math.min(MAX_H, 240) || spaceBelow >= spaceAbove) {
		style.top = `${rect.bottom + 4}px`;
		style.maxHeight = `${Math.max(160, Math.min(MAX_H, spaceBelow))}px`;
	} else {
		style.bottom = `${vh - rect.top + 4}px`;
		style.maxHeight = `${Math.max(160, Math.min(MAX_H, spaceAbove))}px`;
	}
...
function onSearchKeydown(e) {
	if (e.key === "ArrowDown") {
		...
		highlightedIndex.value =
			(highlightedIndex.value - 1 + selectableCount.value) % selectableCount.value;
		nextTick(scrollHighlightedIntoView);
	} else if (e.key === "Enter") {
		e.preventDefault();
		const row = filtered.value.find(
			(r) => r.type === "option" && r._idx === highlightedIndex.value
		);
...
<template>
	<div class="relative">
		...
		<button
			...
			@click="open"
		>
			<span v-if="triggerLabel" class="truncate text-ink-900">{{ triggerLabel }}</span>
		...
		<!-- Popover -->
		<Teleport to="body">
			<div v-if="isOpen" class="fixed inset-0 z-[70]" @click.self="onBackdropClick">
				<div
					:style="popoverStyle"
					class="absolute bg-white border border-ink-200 shadow-fp-lg flex flex-col"
					style="border-radius: 8px; min-width: 260px"
					@click.stop
				>
							...
							<template v-for="(row, i) in filtered" :key="i">
								...
								>
									<span class="min-w-0 flex-1">
										...
										<span
											v-if="row.hint"
											class="text-[11px] text-ink-500 truncate block"
											>{{ row.hint }}</span
										>
									</span>
									<span
										v-if="String(row.value) === String(modelValue)"
										class="text-brand-700 text-xs flex-shrink-0"
										>✓</span
									>
								</button>
							</template>
						</template>
						<div v-else class="px-3 py-3 text-xs text-ink-400 italic text-center">
							No matches.
						</div>
					</div>
				</div>
