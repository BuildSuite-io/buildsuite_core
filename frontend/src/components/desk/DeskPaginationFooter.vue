<script setup>
// Pagination footer for bespoke tables — pairs with usePagination(). Matches DeskList's footer
// (rows-per-page select + range indicator + prev/next), rendered only when there's >1 page.
defineProps({
...
<template>
	...
	>
		...
		<div class="text-ink-500 tabular-nums">
			Showing
			<span class="text-ink-800 font-medium">{{ pager.rangeStart }}–{{ pager.rangeEnd }}</span>
			of <span class="text-ink-800 font-medium">{{ pager.total }}</span>
		</div>
