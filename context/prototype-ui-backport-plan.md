...
| 100 | Approved stages: Delete is approver-only | ✅ | Done with the Stage role-name fix — `STATE_DELETE_ROLES` + `canDelete` gates the Delete button; Approved/Cancelled stages are delete-only for the full (approver) roles. |
...
| 119 | User management: create + edit + persona + emails | ✅ | Built as **real Frappe User** management (not the prototype's localStorage team). Backend `api/users.py` (list/create/update/welcome/reset, admin-gated); persona = the existing `User.persona` Select, synced to a BuildSuite role by the `sync_persona_roles` hook (we only set persona). Frontend: `NewUserView` + rebuilt `UsersView` (real list + edit modal) + `usersApi.js`. Verified on build.local. Map: `admin`→System Manager, `bsa`→BuildSuite Administrator. **Follow-up (deferred):** the app's people pickers (assignee/PM) still use the legacy seed team. See [user-management-scoping.md](user-management-scoping.md). |
