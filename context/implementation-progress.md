...
| 2026-06-18 | **UAT round 2 — PM assignment, inline customer create, ownership-aware gating.** (1) **Project Manager** is now a real assignment: new custom field `project_manager` (Link→**User**) on Project (was writing `owner`, a Frappe no-op). NewProjectView + ProjectEditModal PM pickers switched to `doctype="User"` and write `project_manager`; ProjectDetailView reads/maps/saves it; OverviewTab PM section falls back to the User email/“Project Manager” when `teamMember` can't resolve. (2) **Inline customer create** from the Client picker — new whitelisted `buildsuite_core.api.customers.create_customer` (gated on Project create perm, ERPNext customer_group/territory defaults), `data/customersApi.js` wrapper, shared `CustomerCreateModal.vue`, "+ New" beside the Client field in NewProjectView + ProjectEditModal (picker re-keys to resolve the new value). (3) **Ownership-aware permission gates** — `usePermissions` gained `canEditRecord(doctype, record)` / `canDeleteRecord(doctype, record)`: `'own'`-scope personas (Site Engineer / Foreman) only see Edit/Delete on records they created (resolved via the record's Frappe `owner`, matching the backend gate — fixes the Foreman "error after edit + save"). Wired into TaskDetailView (Start/Mark-complete/Edit/Delete + deps) and TaskProgressEntryDetailView (Edit via `show-save`, Delete via `v-if`); both read transforms now expose `owner`. Stage Planning detail keeps its own workflow-aware gates. Migrate clean; 18/18 UAT tests green; frontend build clean. **Skipped this round (per direction):** PM-task toaster error + TPE below-cumulative validation. |

| 2026-06-18 | **UAT round 3 — monotonic progress, WP auto-status, PM full name.** (1) **TPE progress is now monotonic**: `TaskProgressEntry.validate` rejects (throws, so no log/detail row is created) a `cumulative_progress` below the task's current cumulative (floor = max of sibling entries, excludes self so edits compare against siblings) or outside 0–100. Frontend mirrors it — the File-Progress-Entry modal (TaskDetailView) and NewTaskProgressEntryView both block client-side with an inline error and set the input `min` to the current progress; the server throw also surfaces as a toast. (2) **Work Package status auto-advances** from task progress via `_derive_wp_status` in the existing WP rollup hooks (`update_work_package_progress` + on-trash recalc): Planned→In Progress on any task progress, →Completed when all tasks hit 100%, **On Hold left untouched** (manual). Manual status editing in WorkPackageDetailView already worked and is unchanged — the two coexist. (3) **PM full name** shown in ProjectDetailView: a small `User` lookup resolves `project_manager` → `full_name`, passed to OverviewTab's PM section (email on the sub-line). Tests: `test_uat_fixes.py` now 22 (added monotonic-reject, equal/increase-allowed, WP-autoadvance, WP-on-hold-respected), all green. Code-only — no migrate. |

| 2026-06-18 | **Remove the Owner (creator) column from list surfaces.** Dropped the built-in `owner`-as-person "Owner" column from the **Projects list** (column + cell + fieldOrder + unused UserAvatar import) and the **Work Packages tab** inside Project Detail (WP_COLS entry + cell template; the WP read didn't even fetch owner, so the column rendered an empty avatar). The standalone Work Packages list and WP detail had no Owner column already. Internal `owner` uses are untouched (permission `ownsRecord`, TPE "Entered by", attachment uploader, Task "Assignee" mapping). Note: the Tasks list still has an "Assignee" column (owner-backed) — left as a distinct concept; flag if it should go too. (WP status manual+auto already shipped in the prior commit.) |

| 2026-06-18 | **UAT round 4 — list filters, SE stage submit, modal z-index** (commit `ea11c14`). (1) **TPE list Task filter/column** showed raw IDs: both `entriesResource` and `tasksResource` used the adapter default `pageLength: 20` but the site has 247 tasks, so the name→subject map resolved almost nothing — set `pageLength: 0` (fetch all) on both. (2) **Task list Assignee filter** matched nothing: it was a local `DeskSelect` over `store.team` (member ids) while the backend column is `owner` (user emails). Swapped to a `DeskLinkPicker` (doctype User, enabled-only) so the value is a real user that matches `owner`. (3) **Site Engineer couldn't submit their own Draft stage** — `apply_workflow("Submit for Approval")` threw `PermissionError`. Root cause in `permissions/stage_planning.py`: the own-scope write gate checked the *in-memory* `workflow_state`, but `apply_workflow` sets it to the NEXT state ("Pending Approval") before saving, so the gate's "Draft/Rejected only" rule self-locked the submit. Fixed to check the **persisted** state (`frappe.db.get_value`) — submit succeeds, post-submit edits stay locked. Regression test `test_site_engineer_submits_own_stage_planning`. (4) **Modal pop-ups left the sidebar tab showing as active**: backdrops were `z-40`, below the sidebar's `z-50`. Bumped to `z-[60]` across BoqDetailView, BoqView, RateMasterView, ProjectEditModal, ProjectTeamMemberModal, TaskFormModal. Tests: 23/23 green; frontend build clean. |
